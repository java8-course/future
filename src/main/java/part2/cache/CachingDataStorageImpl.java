...
public class CachingDataStorageImpl<T> implements CachingDataStorage<String, T> {
    ...
    public OutdatableResult<T> getOutdatable(String key) {
        ...
        OutdatableResult<T> outResult = new OutdatableResult<>(res, outdated);
        OutdatableResult<T> casheResult = cache.putIfAbsent(key, outResult);

        if (casheResult != null) {
            return casheResult;
        }

        db.get(key).whenComplete((v, thw) -> {
