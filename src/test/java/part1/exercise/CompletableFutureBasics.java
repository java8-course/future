...
public class CompletableFutureBasics {
    ...
    public void flatMap() throws ExecutionException, InterruptedException {
        ...
        final Optional<Integer> optFirstCodePointOfFirstName = optPerson.flatMap(
                p -> p.getFirstName().codePoints().boxed().findFirst());

        assertEquals(Integer.valueOf(65), optFirstCodePointOfFirstName.get());

        final Stream<Person> streamPerson = Stream.of(person);

        final IntStream codePoints = streamPerson.flatMapToInt(
                p -> p.getFirstName().codePoints());
        ...
        final CompletableFuture<Employee> futureEmployee = futurePerson.thenCompose(
                p -> employeeDb.get(getKeyByPerson(p)));
